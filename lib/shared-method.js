...
function coerceAccepts(uarg, desc) {
  ...
  if (actualType === 'integer' && Number.isNaN(uarg)) {
...
SharedMethod.toResult = function(returns, raw, ctx) {
  ...
  returns = returns.filter(function(item, index) {
    ...
    if (item.root) {
      ...
      result = isFile ? raw[index] : convert(raw[index], targetType);
      return false;
    }
  ...
  returns.forEach(function(item, index) {
    ...
    var value = convert(raw[index], targetType);
    result[name] = value;
  });

  return result;

  function convert(val, targetType) {
    // var targetTypeIsArray = Array.isArray(targetType) &&
    //   targetType.length === 1;

    // if (targetTypeIsArray) targetType = targetType[0];

    var actualType =
      SharedMethod.getType(val, targetType);
    ...
    switch (actualType) {
      ...
      case 'number':
        if (targetType === 'integer')
          return convertToIntegerTargetType(val);
        return val;
      // default:
      //   if (targetType === 'integer') {
      //     return coerceReturns(val, targetType);
      //   }
    }
    return val;
  }
...
// function coerceReturns(raw, targetType) {
//   var data = traverse(raw).forEach(function(x) {
//     var result = x;
//     if (targetType === 'integer') {
//       result = convertToIntegerTargetType(x);
//       this.update(result);
//     }
//     return result;
//   });
...
function convertToIntegerTargetType(val) {
  if (!Number.isInteger(val)) return parseInt(val, 10);
  return val;
}
